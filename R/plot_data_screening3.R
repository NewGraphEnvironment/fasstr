...
plot_data_screening3 <- function(data,
                                dates = Date,
                                values = Value,
  ...
  ##MODIFICATION - add a series mean line with this data
  flow_summary_means <- aggregate(data = flow_summary, Value ~ Statistic, mean)
  flow_summary_means <- dplyr::left_join(dplyr::select(flow_summary, Year, Statistic), 
                                         flow_summary_means, by = "Statistic")
  ...
  sum_plots <- dplyr::mutate(sum_plots,
         ...
         ~ggplot2::ggplot(data = ., ggplot2::aes(x = Year, y = Value)) +
           ...
           geom_line(data = flow_summary_means,
                     aes(x = Year, y = Value)) + ##add a series mean line
